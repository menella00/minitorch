...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    ...
    assert 0.0 <= s <= 1.0
    assert_close(1.0 - s, operators.sigmoid(-a))
    assert_close(operators.sigmoid(0.0), 0.5)
    assert operators.sigmoid(a + 1.0) >= s
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    if operators.lt(a, b) == 1.0 and operators.lt(b, c) == 1.0:
...
def test_other(a: float) -> None:
    ...
    """
    assert_close(operators.add(a, 0.0), a)
